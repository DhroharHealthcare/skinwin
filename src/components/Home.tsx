...
const Hero = ({ setActivePage }: SharedProps) => (
...
<h1 className="relative z-30 text-4xl md:text-6xl font-extrabold tracking-tight text-[#B08A40] mb-6">
  The Art of Radiance, Mastered by Science.
      ...
      <div className="mt-10 flex justify-center gap-4">
        <button onClick={() => setActivePage('services')} className="bg-[#C09A50] text-white font-bold py-3 px-8 rounded-lg shadow-lg hover:bg-[#B08A40] transition duration-300 transform hover:scale-105">
          Discover Our Treatments
        </button>
        <button onClick={() => setActivePage('contact')} className="bg-transparent border-2 border-[#C09A50] text-[#C09A50] font-bold py-3 px-8 rounded-lg hover:bg-[#C09A50] hover:text-white transition duration-300">
...
const SignatureServices = ({ setActivePage }: SharedProps) => (
        ...
        {Object.values(servicesData).map(service => (
          <div key={service.title} className="bg-white rounded-lg shadow-lg overflow-hidden border border-gray-200/50 transform hover:shadow-2xl hover:-translate-y-2 transition-all duration-300">
            ...
            <div className="p-6">
              ...
              <button onClick={() => setActivePage('services')} className="mt-6 text-sm font-semibold text-[#C09A50] hover:text-[#B08A40]">Learn More &rarr;</button>
            </div>
          </div>
        ))}
      </div>
...
const WhyUsTeaser = ({ setActivePage }: SharedProps) => (
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="grid lg:grid-cols-3 lg:gap-x-12 items-center">
        <div className="lg:col-span-1">
          ...
          <button onClick={() => setActivePage('why-us')} className="mt-6 bg-[#C09A50] text-white font-bold py-3 px-6 rounded-lg shadow-lg hover:bg-[#B08A40] transition duration-300">
...
const CallToActionBanner = ({ setActivePage }: SharedProps) => (
  <div className="bg-[#FBF5E9]">
    <div className="max-w-7xl mx-auto py-16 px-4 sm:px-6 lg:px-8 text-center">
      ...
      <button onClick={() => setActivePage('contact')} className="mt-8 w-full inline-flex items-center justify-center px-6 py-3 border border-transparent rounded-md shadow-sm text-base font-medium text-white bg-[#C09A50] hover:bg-[#B08A40] sm:w-auto transition-colors duration-300">
...
const Home = ({ setActivePage }: SharedProps) => {
  return (
    <>
      <Hero setActivePage={setActivePage} />
      <SignatureServices setActivePage={setActivePage} />
      <WhyUsTeaser setActivePage={setActivePage} />
      <Testimonials />
      <CallToActionBanner setActivePage={setActivePage} />
    </>
  );
};
