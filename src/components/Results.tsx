...
interface ResultCategory {
  title: string;
  treatments: ResultTreatment[];
}

// Data for results
const resultsData: Record<string, ResultCategory> = {
    skinCare: {
...
interface BeforeAfterCardProps {
  ...
  resultIndex?: number;
  openModal: (beforeUrl: string, afterUrl: string) => void;
}
      ...
      <div className="grid grid-cols-2 gap-0">
        <div className="relative">
          <img
            ...
            onError={(e: React.SyntheticEvent<HTMLImageElement, Event>) => {
              (e.target as HTMLImageElement).src = `https://placehold.co/400x300/f3f4f6/6b7280?text=Before+${treatment.title.replace(/\s+/g, '+')}`;
            }}
          />
        ...
        <div className="relative">
          <img
            ...
            onClick={() => openModal(beforeImage, afterImage)}
            onError={(e: React.SyntheticEvent<HTMLImageElement, Event>) => {
              (e.target as HTMLImageElement).src = `https://placehold.co/400x300/f3f4f6/6b7280?text=After+${treatment.title.replace(/\s+/g, '+')}`;
            }}
          />
...
const CategorySection = ({ categoryKey, categoryData, isVisible, openModal }: CategorySectionProps) => {
...
const Results = () => {
  ...
  const [modalImages, setModalImages] = useState({ before: '', after: '' });
    ...
    const openModal = (beforeUrl: string, afterUrl: string) => {
