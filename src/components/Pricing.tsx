...
// Complete Package Data with proper savings calculations
const comprehensivePackages: Record<string, Package[]> = {
  skinTransformation: [
    {
      tier: "Premium",
      name: "Glow & Hydrate",
  ...
  hairRestoration: [
    ...
    },
    {
      ...
      technology: "MicroArt® SMP, Precision Microblading, Regen PRP, NeoGraft®",
      resultsTimeline: {
        month2: "PRP boost",
...
const EnhancedPackageCard = ({ package: pkg }: { package: Package }) => {
...
// Your original estimator services (unchanged)
const estimatorServices: Record<string, EstimatorService[]> = {
  facials: [
...
const PriceEstimator = () => {
  const [selectedCategory, setSelectedCategory] = useState<string>('facials');
  const [selectedService, setSelectedService] = useState<EstimatorService>(estimatorServices.facials[0]);
  ...
  const handleServiceChange = (service: EstimatorService) => {
        ...
        <div>
          ...
          <select
            ...
            onChange={(e: React.ChangeEvent<HTMLSelectElement>) => {
              const service = estimatorServices[selectedCategory].find((s: EstimatorService) => s.name === e.target.value);
