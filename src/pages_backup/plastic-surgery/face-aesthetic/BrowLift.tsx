        ...
        <section>
          ...
          <div className="bg-blue-50/50 rounded-2xl p-8 border border-blue-100">
            <ul className="space-y-4 mb-6">
              {["Pre-procedure consultation, photographs & planning", "Anaesthesia or numbing administered (depending on type)", "Incisions made in the hairline (surgical) or Botox injected (non-surgical)", "Forehead muscles and tissues lifted and repositioned", "Excess skin removed (surgical) or muscles relaxed (Botox)", "Incisions closed carefully to minimise scarring"].map((item: any, i: number) => (
            ...
            <div className="grid sm:grid-cols-3 gap-4">
              {[{ label: "Options", value: "Surgical / Non-surgical" }, { label: "Anaesthesia", value: "General / Local" }, { label: "Recovery", value: "1–2 weeks (surgical)" }].map((stat: any, i: number) => (
        ...
        <section>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="bg-green-50/50 rounded-2xl p-8 border border-green-100">
              ...
              <ul className="space-y-3">
                {["Sagging or low-positioned eyebrows", "Heavy brow that makes eyes look tired or angry", "Deep forehead lines and wrinkles", "Frown lines between the eyebrows", "Upper face ageing concerns"].map((item: any, i: number) => (
            ...
            <div className="bg-[#FBF5E9]/50 rounded-2xl p-8 border border-[#C09A50]/20">
              ...
              <ul className="space-y-3">
                {["Eyelid surgery (Blepharoplasty)", "Facelift for complete facial rejuvenation", "Botox for dynamic forehead lines", "Dermal fillers for volume restoration"].map((item: any, i: number) => (
                  <li key={i} className="flex items-start">
                    <span className="h-2 w-2 bg-[#C09A50] rounded-full mt-2 mr-3 flex-shrink-0"></span>
                    <span className="text-gray-700">{item}</span>
                  </li>
        ...
        <section>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="bg-[#FBF5E9]/50 rounded-2xl p-8 border border-[#C09A50]/20">
              ...
              <ul className="space-y-3">
                {["Lifted, naturally arched eyebrows", "Smoother forehead and reduced wrinkles", "More alert and refreshed eye appearance", "Long-lasting results (surgical)", "Natural-looking, balanced outcome"].map((item: any, i: number) => (
                  <li key={i} className="flex items-start">
                    <span className="h-2 w-2 bg-[#C09A50] rounded-full mt-2 mr-3 flex-shrink-0"></span>
                    <span className="text-gray-700">{item}</span>
                  </li>
            ...
            <div className="bg-gray-50 rounded-2xl p-8 border border-gray-200">
              ...
              <ul className="space-y-3">
                {["Temporary swelling and bruising", "Healing time required (surgical)", "Scarring (minimal, hidden in hairline)", "Results may vary from person to person"].map((item: any, i: number) => (
                  <li key={i} className="flex items-start">
        ...
        <section>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="bg-blue-50/50 rounded-2xl p-8 border border-blue-100">
              ...
              <ul className="space-y-3">
                {["Avoid smoking and alcohol before treatment", "Inform the doctor about medications or medical conditions", "Avoid blood-thinning medicines if advised", "Follow all pre-procedure instructions provided"].map((item: any, i: number) => (
                  <li key={i} className="flex items-start">
            ...
            <div className="bg-teal-50/50 rounded-2xl p-8 border border-teal-100">
              ...
              <ul className="space-y-3">
                {["Avoid touching or massaging the treated area", "Avoid strenuous activity and heat exposure initially", "Use prescribed medications and skincare only", "Protect the area from sun exposure", "Attend all follow-up visits as recommended"].map((item: any, i: number) => (
                  <li key={i} className="flex items-start">
