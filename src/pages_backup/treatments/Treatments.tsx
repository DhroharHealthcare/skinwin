...
const CategoryCard = ({ cat, isActive, onClick }: { cat: TreatmentCategory; isActive: boolean; onClick: (id: string) => void }) => (
  <button
...
const TreatmentPanel = ({ cat }: { cat: TreatmentCategory }) => (
  <div className={`bg-gradient-to-br ${cat.color} rounded-2xl border ${cat.border} p-8 sm:p-10`}>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      {cat.groups.map((group: TreatmentGroup, i: number) => (
        <div key={i}>
          <h4 className={`text-xs font-semibold uppercase tracking-wider mb-3 ${cat.accent}`}>
            {group.label}
          </h4>
          <ul className="space-y-1.5">
            {group.items.map((item: TreatmentItem, j: number) => (
              <li key={j}>
                <Link
                  href={item.path}
