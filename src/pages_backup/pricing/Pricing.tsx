...
const EnhancedPackageCard = ({ package: pkg }: { package: any }) => {
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="space-y-3">
                {Object.entries(pkg.resultsTimeline).map(([month, result]: any, idx: number) => (
...
const PriceEstimator = () => {
  ...
  const [selectedService, setSelectedService] = useState<any>(estimatorServices.facials[0]);
  const [selectedQuantity, setSelectedQuantity] = useState(1);
  const [showBreakdown, setShowBreakdown] = useState(false);

  const handleCategoryChange = (category: string) => {
    setSelectedCategory(category);
    setSelectedService((estimatorServices as any)[category][0]);
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {Object.keys(estimatorServices).map((category: any) => (
              <button
                key={category}
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => {
              const service = (estimatorServices as any)[selectedCategory].find((s: any) => s.name === e.target.value);
              handleServiceChange(service);
            }}
            className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#C09A50] focus:border-[#C09A50]"
          >
            {(estimatorServices as any)[selectedCategory].map((service: any) => (
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {getQuantityOptions().map((option: any) => (
              <button
                key={option}
        ...
        {activeTab === 'packages' ? (
          <div>
            ...
            <div className="mb-6">
              <div className="flex justify-center">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 max-w-3xl">
                    {Object.entries(packageCategoryNames).map(([category, name]: [string, string]) => (
                    <button
                      key={category}
                    ...
                    >
                      ...
                      <div className="text-xs opacity-75 mt-1">
                        {(comprehensivePackages as any)[category].length} packages
                      </div>
                    </button>
                  ))}
                </div>
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {(comprehensivePackages as any)[selectedPackageCategory].map((pkg: any, index: number) => (
