...
const resultsData: Record<string, CategoryResult> = {
  skinCare: {
...
interface BeforeAfterCardProps {
  ...
  resultIndex?: number;
  openModal: (before: string, after: string) => void;
}

const BeforeAfterCard = ({ treatment, resultIndex = 1, openModal }: BeforeAfterCardProps) => {
  let beforeImage: string, afterImage: string;
      ...
      <div className="grid grid-cols-2 relative">
        <div className="relative overflow-hidden">
          <img
            ...
            onError={(e: React.SyntheticEvent<HTMLImageElement, Event>) => { (e.target as HTMLImageElement).src = `https://placehold.co/400x300/f3f4f6/9ca3af?text=Before`; }}
          />
        ...
        <div className="relative overflow-hidden">
          <img
            ...
            className="w-full h-52 object-cover group-hover:scale-105 transition-transform duration-500"
            onClick={() => openModal(beforeImage, afterImage)}
            onError={(e: React.SyntheticEvent<HTMLImageElement, Event>) => { (e.target as HTMLImageElement).src = `https://placehold.co/400x300/f3f4f6/9ca3af?text=After`; }}
          />
...
const CategorySection = ({ categoryData, isVisible, openModal }: CategorySectionProps) => {
...
const Results = () => {
  ...
  const openModal = (before: string, after: string) => { setModalImages({ before, after }); setModalOpen(true); };
